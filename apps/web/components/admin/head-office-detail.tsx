...
import { Building2, PlusCircle, X, Power, Link2, Search, Layers, MapPin } from "lucide-react";
...
export function HeadOfficeDetail({ headOffice }: { headOffice: HeadOfficeData }) {
  ...
  const [tab, setTab] = useState<"dealerships" | "sites">("dealerships");
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          ...
          <div className="flex gap-1 rounded-lg border border-slate-200 bg-slate-50 p-0.5">
            <button
              onClick={() => { setTab("dealerships"); setShowForm(false); setShowAttach(false); }}
              className={cn("flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-semibold transition",
                tab === "dealerships" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700")}
            >
              <Building2 className="h-3.5 w-3.5" /> Dealerships
              <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">{dealerships.length}</span>
            </button>
            <button
              onClick={() => { setTab("sites"); setShowForm(false); setShowAttach(false); }}
              className={cn("flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-semibold transition",
                tab === "sites" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700")}
            >
              <Layers className="h-3.5 w-3.5" /> Sites
              <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">
                {dealerships.reduce((n, d) => n + (d.sites?.length ?? 0), 0)}
              </span>
            </button>
          </div>
          {tab === "dealerships" && (
