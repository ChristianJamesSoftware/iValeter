...
import { CalendarTab, JobsTab, SiteTabBar, type SiteTab } from "./site-calendar-jobs";
...
function LiveBoard({
  ...
  return (
    <div>
      ...
      {/* Team leader visit sign-off */}
      <div className={cn(
        "mb-5 flex items-center justify-between rounded-xl border px-5 py-3",
        visitSigned ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50",
      )}>
...
export function OpsCentre({ sites, valeters }: { sites: SiteOpt[]; valeters: ValeterOpt[] }) {
  ...
  const [activeTab, setActiveTab]     = useState<SiteTab>("live");
  ...
  return (
    <div>
      ...
      {/* Tab bar: Live Board | Calendar | All Jobs */}
      <SiteTabBar active={activeTab} onChange={setActiveTab} />

      {/* Tab panels */}
      {activeTab === "live" && (
        <LiveBoard
          siteId={siteId}
          siteValeters={siteValeters}
          onAssign={(id) => setAssignBookingId(id)}
        />
      )}
      {activeTab === "calendar" && <CalendarTab siteId={siteId} />}
      {activeTab === "jobs"     && <JobsTab     siteId={siteId} />}
